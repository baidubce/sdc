...
func main() {
	...
	if len(ak) == 0 {
		fmt.Println("请输入ak参数，例如 71Of*********fgTx 获取地址:" + appListURL)
	...
	if len(sk) == 0 {
		fmt.Println("请输入sk参数，例如 lkFM*********hPqH")
	...
	// 菜谱大模型接口
	controller.GetRecipeInfo("/ernie/recipe", authorizedRouterGroup, modelDispatcher)
	controller.GenerateRecipeImage("ernie/recipeImg", authorizedRouterGroup, ak, sk, modelDispatcher)
	// 打包FE资源
