...
// 只返回ErnieBot 对象
func NewBot(ak, sk, version string) (*ErnieBot, error) {
	model := &ErnieBot{ak: ak, sk: sk, appVersion: version, URL: ErnieModelMap}
	token, e := model.getToken()
...
// 调用文生图接口生成图片
func (eb *ErnieBot) Txt2imgSD(imgParams string) (string, error) {
	// 定义请求地址
	host := "https://aip.baidubce.com/rpc/2.0/ai_custom/v1/wenxinworkshop/text2image/sd_xl"
	url := fmt.Sprintf("%s?access_token=%s&sourceVer=%s&source=app_center&appName=online", host, eb.token, eb.appVersion)
	payload := strings.NewReader(imgParams)
	// 请求超时时间 60s
	timeout := 60 * time.Second
	httpClient := http.Client{Timeout: timeout}
	httpResp, e := httpClient.Post(url, "application/json", payload)

	if e != nil {
		return "", e
	}
	respText, _ := io.ReadAll(httpResp.Body)
	respObj := gjson.Parse(string(respText))

	if respObj.Get("error_code").Int() != 0 {
		return "", errors.New(string(respText))
	}
	png := respObj.Get("data.0.b64_image").String()
...
func (eb *ErnieBot) getServerUrl(model domain.ModelType) string {
	...
	return fmt.Sprintf("%s%s?access_token=%s&sourceVer=%s&source=app_center&appName=chatDemo", host, modelKey, eb.token, eb.appVersion)
}
