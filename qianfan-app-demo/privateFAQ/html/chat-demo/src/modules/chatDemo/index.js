...
import { Input, toast, DialogBox, Tooltip, Modal } from 'acud';
...
export default ({appName}) => {
  ...
  const cUpLoads = useRef(null);

  const [isTypewriter, setIsTypewriter] = useState(false);
  const [isScroll, setIsScroll] = useState(true);

  const [isSetSywd, setIsSetSywd] = useState(true);



  const [recommendList, setRecommendList] = useState([])

  const [upCompSucStatus, setUpCompSucStatus] = useState(false) //判断上传组件是否传成功了


  const [isComposing, onCompositionStart, onCompositionEnd] = useInputHooks()

  const [isShowMask, setIsShowMask] = useState(false); //控制引导蒙版
  ...
  const reqFn = async (list) => {
    ...
    let historyRawMsg = "";
    eventSourceObj = new EventSourcePolyfill(`/v1/ernie/chat?isPrivate=${isSetSywd ? '1' : '0'}&` + (new URLSearchParams(params)).toString(), {
      headers: {
        'Req-UUID': uuid
      }
    });
    eventSourceObj.onmessage = function (event) {
      ...
      if (res instanceof Object && res.err_no === 0) {
        ...
        if (res.data.is_end) {
          ...
          handleHistoryData(list_copy, res.data.usage, uuid,localKey); // 更新储存的历史信息
  ...
  const getEmbeddingSuggest = async () => {
    let res = await Api.getEmbeddingSuggest({ random: 1 })
  ...
  return (
    <Fragment>
      <div className="chatDemo">
        <Top isSetSywd={isSetSywd} modalValue={modalValue} setModalValue={setModalValue} isShowMask={isShowMask} setIsShowMask={setIsShowMask} getSywd={e => { setIsSetSywd(e) }} changeContent={changeContent} currentDom_h5={currentDom_h5} />
        <div className="mainBox">
          <div className="forLeftHidden">
            <Left
              ...
              upCompSucStatus={upCompSucStatus}
              cUpLoads={cUpLoads}
          ...
          <div className="rightBox">
            <div className="msgBox">
                ...
                {messageList.length === 0 && (
                  <div className="defaultRab">
                    ...
                    <div className="textBox">
                      ...
                      <div className="desc">您可以从左侧『当前已导入语料信息』中选择提问示例进行提问，也可以在下方输入框中直接输入您的问题</div>
                      {
                        recommendList.length > 0 ? <div className="desc change">
                          <span>您可以试着问：</span>
                          <span onClick={e => getEmbeddingSuggest()}>
                            <i></i>
                            换一换</span>
                        </div> : <div className="desc">
                          还未导入语料信息，<span className='upBtn' onClick={e => cUpLoads.current.setIsShowUpPop(true)}>点击导入</span>
                        </div>
                      }
                      <div className="embeddingSuggestsList">
                        {
                          recommendList.map(v => {
                            return <div className="item" onClick={e => setTextAreaVal(v.Q)}>
                              {v.Q}
                            </div>
                          })
                        }
              ...
              <div className="msgSend">
                <div className="desc">您可以尝试输入任何问题，或在左侧当前已导入语料信息中选择您感兴趣的关键词</div>
                <div className="forBorder">
                  <div className="textAreaBox">
                    <UpLoads
                      isShowMask={isShowMask}
                      setIsShowMask={setIsShowMask}
                      modalValue={modalValue}
                      isSetSywd={isSetSywd}
                      MD={md}
                      Prism={Prism}
                      getEmbeddingSuggest={getEmbeddingSuggest}
                      setUpCompSucStatus={setUpCompSucStatus}
                      cUpLoads={cUpLoads}
                    />
                    <Input.TextArea
                      placeholder="请输入问题"
